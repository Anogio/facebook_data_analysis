...
def all_messages_over_time(messages_df, interval):
    ...
    rolling_n_messages = messages_df.set_index(messages_cols.date).sort_index()['one'].rolling(interval).sum().rename(
        amount_column_name).reset_index()
...
def one_person_graphs(messages_df, name, interval='7d', top_n_pie=10):
    ...
    messages_by_conversation = person_df.groupby(messages_cols.conversation)[messages_cols.sender].count()
    top_messages_by_conversation = messages_by_conversation.sort_values().iloc[-top_n_pie:]
    pie = pd.DataFrame(top_messages_by_conversation.rename('n_messages')).plot(
        kind='pie', y='n_messages', title='{} (top {} conversations)'.format(name, top_n_pie), legend=False
    )

    amount_column_name = 'n_messages_per_' + interval
    person_df['one'] = 1
    line = person_df.set_index(messages_cols.date).sort_index()['one'].rolling(interval).sum().rename(
        amount_column_name).reset_index().plot(x='date', y=amount_column_name, title=name)
